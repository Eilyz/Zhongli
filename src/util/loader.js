...
import { Zhongli } from '../Zhongli.js'
...
export default {
  loadCommands: async function () {
    ...
    for (const file of commandFiles) {
      ...
      Zhongli.commands.set(command.data.name, command)
  ...
  loadEvents: async function () {
    ...
    for (const file of eventFiles) {
      ...
      if (event.once) {
        Zhongli.once(event.name, (...args) => event.execute(...args))
      } else Zhongli.on(event.name, (...args) => event.execute(...args))
    }
  },
}
